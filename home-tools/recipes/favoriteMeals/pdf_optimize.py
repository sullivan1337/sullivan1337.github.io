...
def optimize_pdf_images(pdf_path, quality=92):
    ...
    for page_num in range(len(doc)):
        ...
        for img_index, img in enumerate(images):
            ...
            image_rect = fitz.Rect(img[2], img[3], img[2] + img[4], img[3] + img[5])
            page.insert_image(image_rect, stream=optimized_image_io.getvalue())

    # Save the optimized PDF
    optimized_pdf_path = os.path.splitext(pdf_path)[0] + "_optimized.pdf"
    doc.save(optimized_pdf_path, garbage=4, deflate=True)
    doc.close()

...
async function generateRegoPolicy() {
    ...
    const prompt = `Write a Rego policy using the user-q and JSON input "user-q":${userQuery}\n"JSON":${formattedJson}\n`;
    const response = await fetchChatGPT(prompt, apiKey);
    regoOutput.value = response.choices[0].text.trim();

}

async function fetchChatGPT(prompt, apiKey) { // Add apiKey parameter to the function
    const url = "https://api.openai.com/v1/engines/text-davinci-002/completions";
    const headers = {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${apiKey}`
    ...
    const conversationHistory = `user: [Use OPA's rego policy language

    Rego Rule guidelines:
    1. Rule must start with \`package wiz\`
    ...
    4. The results should return \`pass\` or \`fail\`
    5. The following is an example of a working rego rule: 
    \`\`\`
    package wiz

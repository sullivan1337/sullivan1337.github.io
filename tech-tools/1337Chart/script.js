...
document.addEventListener('DOMContentLoaded', () => {
  ...
  btnSubmitEntry.addEventListener('click', () => {
    ...
    // If a LinkedIn link is provided but no token is available,
    // we do a mock (since LinkedIn doesn’t allow fetching arbitrary user data)
...
function checkForLinkedInRedirect() {
  ...
  if (code && state) {
    // Retrieve stored code_verifier and state from sessionStorage
    ...
    if (codeVerifier) {
      exchangeCodeForToken(code, codeVerifier)
        .then(token => {
          ...
          // Note: If you encounter a 404 here, try using "https://api.linkedin.com/v2/me" instead.
          fetchLinkedInUserInfo()
            .then(userinfo => {
              console.log("Fetched LinkedIn UserInfo:", userinfo);
              // For example, you might auto-populate form fields here.
            })
...
async function fetchLinkedInUserInfo() {
  ...
  // If you get a 404 here, replace /userinfo with /me
...
function autoLayout() {
  ...
  // BFS for layout assignment
  const queue = [];
  roots.forEach((root, i) => {
    queue.push({ personId: root.id, level: 0, siblingIndex: i });
  });
  ...
  while (queue.length) {
    ...
    kids.forEach((kidId, idx) => {
      queue.push({ personId: kidId, level: level + 1, siblingIndex: idx });
    });
  }
  ...
  Object.keys(levelsMap).forEach(levelStr => {
    ...
    levelsMap[level].forEach(({ personId, siblingIndex }) => {
      const person = people.find(p => p.id === personId);
      if (person) {
        person.x = siblingIndex * colWidth;
        person.y = level * rowHeight;
      }
    });
  });
  ...
  if (minX < 0) {
    people.forEach(p => { p.x += Math.abs(minX) + 50; });
  }
  if (minY < 0) {
    people.forEach(p => { p.y += Math.abs(minY) + 50; });
  }
...
function renderAllCards() {
  ...
  Array.from(chartArea.getElementsByClassName('person-card')).forEach(card => card.remove());
...
function getCardCenter(personId) {
  const chartArea = document.getElementById('chartArea');
  const card = Array.from(chartArea.getElementsByClassName('person-card')).find(el => {
  ...
  return {
    x: rect.left + rect.width / 2 - chartRect.left + chartArea.scrollLeft,
    y: rect.top + rect.height / 2 - chartRect.top + chartArea.scrollTop
  };
}
...
/****************************************************
 * 11) MISC: CLEAR & POPULATE MODAL FIELDS
 ****************************************************/

...
Module["onRuntimeInitialized"] = function() {
    Module.promised = {};
    Module.promised.json = function(input, program) {
        return new Promise(function(resolve, reject) {
            try {
                var inputPtr = Module._malloc(Module.lengthBytesUTF8(JSON.stringify(input)) + 1);
                var programPtr = Module._malloc(Module.lengthBytesUTF8(program) + 1);
                var resultPtr = Module._malloc(1024 * 1024); // 1MB buffer for result
                var resultLengthPtr = Module._malloc(4); // Pointer to result length

                Module.stringToUTF8(JSON.stringify(input), inputPtr, Module.lengthBytesUTF8(JSON.stringify(input)) + 1);
                Module.stringToUTF8(program, programPtr, Module.lengthBytesUTF8(program) + 1);

                var resultCode = Module._jq_run(inputPtr, programPtr, resultPtr, resultLengthPtr);
                if (resultCode !== 0) {
                    throw new Error(Module.UTF8ToString(resultPtr));
                }

                var resultLength = Module.getValue(resultLengthPtr, "i32");
                var result = Module.UTF8ToString(resultPtr, resultLength);

                Module._free(inputPtr);
                Module._free(programPtr);
                Module._free(resultPtr);
                Module._free(resultLengthPtr);
